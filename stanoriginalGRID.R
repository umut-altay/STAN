...
inla.setOption(pardiso.license = "simulation_grid/pardiso.lic")
...
kenya.geo <-readOGR('simulation_grid/geodata/KEGE71FL.shp')
...
kenya_shape=readOGR('simulation_grid/kenya_administrative1/ken_admbnda_adm1_iebc_20191031.shp')
...
xx_obs = seq(min(kenya.geo$LONGNUM), max(kenya.geo$LONGNUM), length.out = 14)
yy_obs = seq(min(kenya.geo$LATNUM), 5.5, length.out = 14) #reason for 5.5 : max(kenya.geo$LATNUM) can't capture part of the North-West upper tip of the Kenya map
loc.obs = cbind(rep(xx_obs, each = length(yy_obs)), rep(yy_obs, length(xx_obs)))

#Removing the points (that are outside the Kenya) from the grid

#Convert observation points grid into a SpatialPointsDataFrame object 
grid=data.frame(xCoor = loc.obs[,1], yCoor = loc.obs[, 2])
grid <- SpatialPointsDataFrame(grid, data.frame(id=1:length(grid[,1]))) #we have 2500 points in the initial grid

#Remove the locations that are out of Kenya
grid=erase.point(grid, kenya_shape, inside = FALSE)

#Make a new loc.pred matrix from the remaining points
loc.obs = cbind(grid@coords[ ,1], grid@coords[ ,2])
nLoc = dim(loc.obs)[1]
...
save(myData, u.sim, kenya.geo, file="simulation_grid/myDataOriginalGrid.RData")
...
save(res.inla, res.inla.hyper, stk.e, stk.pred, stk.full, file="simulation_grid/INLA_originalGrid.RData")
...
saveRDS(res_stan, file = "simulation_grid/STAN_new_originalGrid.RDS")
...
save(res_stan, data.stan, uSample, thetaSample, sdSpatialSample, rangeSample, sdNuggetSample, etaSample, file="simulation_grid/STAN_new_originalGrid.RData")
...
# Compile STAN code
model_stan = stan_model("simulation_grid/stan_old.stan")
...
saveRDS(res_stan, file = "simulation_grid/STAN_old_originalGrid.RDS")
...
save(start_all, start_INLA, end_INLA, start_newSTAN, end_newSTAN, start_sampNewSTAN, end_sampNewSTAN, start_oldSTAN, end_oldSTAN, start_sampOldSTAN, end_sampOldSTAN, end_all, file="time_GridOriginal.RData")

#Save results from analysis with old STAN file
save(res_stan, data.stan, uSample, thetaSample, sdSpatialSample, rangeSample, sdNuggetSample, etaSample, file="simulation_grid/STAN_old_originalGrid.RData")
